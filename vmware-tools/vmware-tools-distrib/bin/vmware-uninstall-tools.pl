...
my $cInstallerFileNameReal = 'vmware-install.real.pl';
   ...
   if (vmware_product() eq 'tools-for-linux') {
      if (getKernRelInteger() >= kernel_version_integer(4, 0, 0)) {
         ...
         if (($vmtoolsd_version_string =~ /version ([\d+\.]+)/) and
            ...
            my $ans;
            $ans = get_answer('For kernels >= 4.0.0, open-vm-tools need to be of ' .
                              'version 10.0.0 or higher for VMware Host Guest Filessystem ' .
                              '(vmhgfs) to function with VMware Tools. ' .
                              'Please upgrade open-vm-tools and rerun this installer, ' .
                              'or uninstall open-vm-tools. Alternatively, you can disable using ' .
                              'the vmhgfs when VMware Tools are configured.' . "\n\n" .
...
sub main {
   ...
   if (internal_basename($0) eq $cInstallerFileName or
       internal_basename($0) eq $cInstallerFileNameReal) {
     my $answer;
    ...
    if (vmware_product() eq 'tools-for-linux') {
	  ...
	  if (%os) {
		...
		my $ans = get_answer(
			     "open-vm-tools are available from the OS vendor and " .
			     "VMware recommends using open-vm-tools. " .
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '10.0.5') and
